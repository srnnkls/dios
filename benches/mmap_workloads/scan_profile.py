...
def profile(options: argparse.Namespace) -> None:
    ...
    if manifest["status"] != "complete" or manifest["mode"] != "run":
        raise ValueError("profile requires completed primary samples")
    eligible = {case[arm] for case in manifest["comparisons"] for arm in ("base", "candidate")}
    if options.config not in eligible or not 1 <= options.repetitions <= 128:
        raise ValueError("unmeasured configuration or excessive profile repetitions")
    ...
    launch = prefix + [shutil.which("bash"), "-c", wrapper, "scan-profile", str(scratch / "pid"),
                       str(binary), "scan-profile", manifest["input"], options.config,
    ...
    for path in sorted((output / "samples").glob("*.json")):
        validate_scan(json.loads(path.read_text()))
    metadata = dict(configuration=options.config, event=event, sample_unit="samples",
                    ...
                    repetitions=options.repetitions, executable_sha256=manifest["executable_sha256"],
